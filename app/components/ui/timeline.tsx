    ...
    >
      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-20">
        {data.map((item, index) => (
          <div
            key={index}
            className="flex justify-start pt-4 md:pt-8 md:gap-10"
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              <div className="h-6 absolute left-1 md:left-3 w-6 md:h-10 md:w-10 rounded-full bg-background flex items-center justify-center">
                <div className="h-2 w-2 md:h-4 md:w-4 rounded-full bg-muted border border-border p-1 md:p-2" />
              </div>
        ...
        >
          <motion.div
            ...
            className="absolute inset-x-0 top-0  w-[2px] bg-gradient-to-t from-orange-600 via-orange-400 to-transparent from-[0%] via-[10%] rounded-full dark:from-orange-400 dark:via-orange-200"
          />
        </div>
      </div>
    </div>
